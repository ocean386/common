...
func (s *Sharding) registerCallbacks(db *gorm.DB) {
	s.Callback().Create().Before("*").Register(s._config.Name, s.switchConn)
	s.Callback().Query().Before("*").Register(s._config.Name, s.switchConn)
	s.Callback().Update().Before("*").Register(s._config.Name, s.switchConn)
	s.Callback().Delete().Before("*").Register(s._config.Name, s.switchConn)
	s.Callback().Row().Before("*").Register(s._config.Name, s.switchConn)
	s.Callback().Raw().Before("*").Register(s._config.Name, s.switchConn)
}
...
func (s *Sharding) resolve(query string, args ...interface{}) (ftQuery, stQuery, tableName string, err error) {
	...
	if isInsert {
		...
		case *sqlparser.SelectStatement:
			...
			stmt.Condition = replaceConditionTableName(stmt.Condition, tableName, newTable.Name.Name)
			stQuery = stmt.String()
		case *sqlparser.UpdateStatement:
			ftQuery = stmt.String()
...
func (s *Sharding) insertValue(keys string, names []*sqlparser.Ident, exprs []sqlparser.Expr, args ...interface{}) (value interface{}, id int64, keyFind bool, err error) {
	for index, key := range strings.Split(keys, ",") {
		...
		if !keyFind && index == len(strings.Split(keys, ","))-1 {
...
func (s *Sharding) nonInsertValue(keys string, condition sqlparser.Expr, args ...interface{}) (value interface{}, id int64, keyFind bool, err error) {
	...
	for index, key := range strings.Split(keys, ",") {
		err = sqlparser.Walk(sqlparser.VisitFunc(func(node sqlparser.Node) error {
			if n, ok := node.(*sqlparser.BinaryExpr); ok {
				...
				if x, ok := n.X.(*sqlparser.QualifiedRef); ok {
					if x.Column.Name == key && n.Op == sqlparser.EQ {
						keyFind = true
						switch expr := n.Y.(type) {
						case *sqlparser.BindExpr:
							value = args[expr.Pos]
						...
						return nil
					} else if x.Column.Name == "id" && n.Op == sqlparser.EQ {
						switch expr := n.Y.(type) {
						case *sqlparser.BindExpr:
							v := args[expr.Pos]
							var ok bool
		...
		if !keyFind && id == 0 && index == len(strings.Split(keys, ","))-1 {
			return nil, 0, keyFind, ErrMissingShardingKey
		}
	}
...
func replaceConditionTableName(e sqlparser.Expr, oldName, newName string) sqlparser.Expr {
	if be, ok := e.(*sqlparser.BinaryExpr); ok {
		...
		if x, ok := be.X.(*sqlparser.BinaryExpr); ok {
			be.X = replaceConditionTableName(x, oldName, newName)
		}
		if y, ok := be.Y.(*sqlparser.QualifiedRef); ok {
			y.Table.Name = newName
			be.Y = y
			e = be
		}
		if y, ok := be.Y.(*sqlparser.BinaryExpr); ok {
			be.Y = replaceConditionTableName(y, oldName, newName)
		}
	}
	return e
}
